...
interface DataProps {
  ...
  bookingData: any;
  onSeatSelected: (newSelectedSeats: { seatNumber: string; travellerId: string; class: 'economyClass' | 'businessClass' | 'firstClass' }[]) => void;
}
...
const SeatLayout: React.FC<DataProps> = ({
  ...
  const [selectedSeats, setSelectedSeats] = useState<{ seatNumber: string; travellerId: string; class: 'economyClass' | 'businessClass' | 'firstClass' }[]>([]);
  ...
  if (chartError) {
    return <div className="flex justify-center items-center h-64 text-red-500">Error loading seat layout</div>;
  }
  ...
  const handleSeatClick = (seatNumber: string) => {
    const existingSeatIndex = selectedSeats.findIndex(seat => seat.seatNumber === seatNumber);
    console.log(travellers);
    
    if (existingSeatIndex !== -1) {
      const newSelectedSeats = selectedSeats.filter(seat => seat.seatNumber !== seatNumber);
      setSelectedSeats(newSelectedSeats);
      onSeatSelected(newSelectedSeats);
    } else if (selectedSeats.length < travellers.length) {
      const availableTravellerId = travellers.find(traveller => 
  ...
  const renderSeat = (seat: Seat) => {
    const isSelected = selectedSeats.some(selectedSeat => selectedSeat.seatNumber === seat.number);

    return (
      <div
        className={`w-8 h-12 flex items-center justify-center text-xs font-bold  rounded
          ${seat.isAvailable && !isSelected ? 'bg-blue-500 border-blue-500 border-2 text-white cursor-pointer hover:bg-blue-300' : ''}
          ${seat.isAvailable && isSelected ? 'bg-white text-green-500 border-2 border-green-500 cursor-pointer' : ''}
    ...
    for (let i = 0; i < seats.length; i++) {
      ...
      if (seats[i].position === 'aisle') {
        if (i < seats.length - 1 && seats[i + 1].position === 'aisle') {
          ...
          currentGroup = 0;
        } else if (i === seats.length - 1 || seats[i + 1].position === 'window') {
          config.push(currentGroup);
          currentGroup = 0;
        }
      }
      ...
      <div className="flex mb-3 items-center">
        {configuration.map((groupSize, groupIndex) => (
          <React.Fragment key={groupIndex}>
            {groupIndex > 0 && <div className="w-8 " />} {/* Space between seat groups */}
            <div className="flex">
              {Array.from({ length: groupSize }).map((_, seatInGroupIndex) => {
                ...
                return (
                  <React.Fragment key={seatInGroupIndex}>
                    {renderSeat(seat)}
                    {seatInGroupIndex < groupSize - 1 && <div className="w-2" />} {/* Small gap between seats in a group */}
                  </React.Fragment>
                );
              })}
            </div>
    ...
    return (
      <div className=" bg-transparent mr-9">
        ...
        <div className="bg-white">
          <h3 className="text-lg font-bold mb-4 text-center capitalize">{seatClass.replace('Class', ' Class')}</h3>
          <div className="flex flex-col items-center">
            {scheduleData.seatLayout[seatClass].map((row: Row, rowIndex: number) => (
          ...
          <div key={index} className="m-5 items-center ">
            <div className="flex justify-between items-center">
              <div className="flex justify-start space-x-1">
                <h4 className="text-base font-bold ">{traveller.firstName || ''} </h4>
                <h4 className="text-base font-bold ">{traveller.lastName || ''} </h4>
              </div>
              {selectedSeats.find(seat => seat.travellerId === traveller._id) ? (
                <p className="px-4 bg-white border text-sm border-black rounded">
                  {selectedSeats.find(seat => seat.travellerId === traveller._id)?.seatNumber}
                </p>
              ) : (
                <p className="">none</p>
              )}
