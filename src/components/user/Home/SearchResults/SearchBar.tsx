...
import Tooltip from '../Homepage/Modals/ToolTip';
...
function SearchBar() {
  ...
  const selectedValue = useSelector(
    (state: RootState) => state.HeroAuth.selectedValue
  );
  const classState = useSelector(
    (state: RootState) => state.HeroAuth.classState
  );
  const fromAirport = useSelector(
    (state: RootState) => state.HeroAuth.fromAirport
  );
  const toAirport = useSelector((state: RootState) => state.HeroAuth.toAirport);
  const departureDate = useSelector(
    (state: RootState) => state.HeroAuth.departureDate
  );
  const returnDate = useSelector(
    (state: RootState) => state.HeroAuth.returnDate
  );
    ...
    <div className="ml-[10%] font-PlusJakartaSans ">
      ...
      >
        ...
        <div className="flex items-start flex-wrap ">
          ...
          <button
            type="submit"
            className="absolute top-[24px] right-[200px] text-xl py-2 font-PlusJakartaSans rounded-xl font-extrabold  transition-all ease-in-out duration-300 h-16  bg-gradient-to-r from-pink-400 to-purple-600  text-white px-8 "
          >
            SEARCH
          </button>
        </div>
