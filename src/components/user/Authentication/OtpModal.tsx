...
const OtpModal: React.FC<OtpModalProps> = ({ closeModal ,openModal }) => {
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (response.data.success) {
          ...
          // dispatch(userLogin({ token, user_data }));
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50">
      <div className="bg-white p-8 shadow-lg w-1/3 rounded-lg">
        ...
        <p className="text-base mb-4 text-center font-normal">Enter the OTP sent to {userEmail}</p>
