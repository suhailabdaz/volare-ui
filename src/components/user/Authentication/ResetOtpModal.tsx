...
const ResetOtpModal: React.FC<OtpModalProps> = ({ closeModal ,openModal }) => {
  ...
  function obfuscateEmail(email:string) {
    ...
    const starCount = localPart.length-visibleLocalPart.length

    const stars = '*'.repeat(starCount);
  
    const obfuscatedEmail = `${visibleLocalPart}${stars}@${domainPart}`;
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50">
      <div className="bg-white p-8 shadow-lg w-1/3 rounded-lg">
        ...
        <h2 className="text-2xl mb-1 text-center font-bold">Enter OTP to Reset Password</h2>
        <p className="text-base mb-4 text-center font-normal">Enter the OTP sent to {obfuscateEmail(userEmail||"")}</p>
