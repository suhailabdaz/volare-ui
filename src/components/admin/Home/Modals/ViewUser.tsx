...
const ViewUser: React.FC<ProfileModalProps> = ({ closeModal, userId }) => {
  const [showConfirmModal, setShowConfirmModal] = useState(false);

  const foundUser = useSelector((state: RootState) => {
    const users = state.AdminAuth.users;
  ...
  var initialValues = {
    ...
    status: foundUser?.status ? 'Active' : 'Blocked',
    gender: foundUser?.gender || '',
    address: foundUser?.address || '',
    birthday: foundUser?.birthday
      ? new Date(foundUser.birthday).toISOString().split('T')[0]
      : '',
  ...
  const handleDelete = async () => {
    if (foundUser?._id) {
      ...
      if (response.data.success) {
        toast.message(`User ${foundUser?.status ? 'Blocked' : 'Unblocked'}`,{
      ...
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white px-8 py-6 shadow-lg w-[35%] border border-black max-h-[80%] overflow-hidden">
          <div className="flex justify-between items-center mb-4">
            ...
            >
              {foundUser?.status ? 'Block' : 'Unblock'}
            </button>
          </div>
          <Formik
