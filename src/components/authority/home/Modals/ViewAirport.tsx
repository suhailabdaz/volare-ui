...
import { removeAirport, setAirportDetails } from '../../../../redux/slices/authoritySlice';
...
const ViewAirport: React.FC<ProfileModalProps> = ({ closeModal, openModal,airportId }) => {
  ...
  const foundAirport = useSelector((state: RootState) => {
    ...
    return travellers.find((traveller) => traveller._id === airportId);
  }); 
  ...
  const onSubmit = async (values: typeof initialValues, { setSubmitting }: { setSubmitting: (isSubmitting: boolean) => void }) => {
    try {
      ...
      const response = await createAxios().post(authorityEndpoints.saveAirport, values);
  ...
  const handleDelete = async()=>{
    if(foundAirport?._id){
      const response = await createAxios().delete(`${authorityEndpoints.deleteAirport}/${foundAirport._id}`);
      if(response.data.success){
      ...
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white px-8 py-6 shadow-lg w-[35%] border border-black max-h-[80%] overflow-hidden">
          <div className=' flex justify-between items-center mb-4'>
            <h2 className="text-3xl text-left font-PlayfairDisplay font-bold mb-3">Edit Airport Info</h2>
            <button onClick={()=>handleDelete()} className='border-2 border-black font-lg text-black text-base p-1 font-bold hover:scale-105 transition-all ease-in-out duration-150 '>Delete</button>
          </div>
                ...
                <div id="basic-info" className="mb-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="flex flex-col">
                      <label htmlFor="airport_code" className="mb-1 text-sm font-semibold">Airport Code <span className='text-red-900'>*</span></label>
                      <Field
                        type="text"
                        name="airport_code"
                        id="code"
                      ...
                      />
                      <ErrorMessage name="airport_code" component="div" className="text-red-500 text-xs mt-1" />
                    </div>

                    {/* Last Name */}
                    <div className="flex flex-col">
                      <label htmlFor="airport_name" className="mb-1 text-sm font-semibold">Airport Name <span className='text-red-900'>*</span></label>
                      <Field
                        type="text"
                        name="airport_name"
                        id="airport_name"
                        className="p-3 border border-gray-500 rounded-lg"
                      />
                      <ErrorMessage name="airport_name" component="div" className="text-red-500 text-xs mt-1" />
                    </div>

                    {/* Gender */}
                    <div className="flex flex-col">
                      <label htmlFor="city" className="mb-1 text-sm font-semibold">City <span className='text-red-900'>*</span></label>
                      <Field
                        type="text"
                        name="city"
                        id="city"
                    ...
                    {/* Date of Birth */}
                    <div className="flex flex-col">
                      <label htmlFor="country" className="mb-1 text-sm font-semibold">Country <span className='text-red-900'>*</span></label>
                      <Field
                        type="text"
                        name="country"
                        id="country"
