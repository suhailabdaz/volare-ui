...
const AddAirport: React.FC<ProfileModalProps> = ({ closeModal, openModal }) => {
  ...
  const onSubmit = async (values: typeof initialValues, { setSubmitting }: { setSubmitting: (isSubmitting: boolean) => void }) => {
    try {
      ...
      const response = await createAxios().post(authorityEndpoints.addAirport, values);
      ...
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white px-8 py-6 shadow-lg w-[35%] border border-black max-h-[80%] overflow-hidden">
          <div className=' justify-between items-center mb-4'>
            <h2 className="text-3xl text-left font-PlayfairDisplay font-bold mb-3">Add Airport Info</h2>
          </div>
